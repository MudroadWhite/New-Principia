...
Proof.
  assert (Sa : (P → R) ∧ (Q → S) → P ∧ Q → Q ∧ R).
  {
    ...
    replace (R∧Q) with (Q∧R) in Fact3_45a by now apply (and_comm Q R).
  ...
  assert (Sb : (P → R) ∧ (Q → S) → Q ∧ R → R ∧ S).
  {
    ...
    replace (S∧R) with (R∧S) in Fact3_45b by now apply (and_comm R S).
    Syll Simp3_27a Fact3_45b Sb. (*Syll2_05*)
    exact Sb.
  }
  Conj Sa Sb H.
  (* Useless code being commented out. I don't know what does this do *)
  (* split.
    apply Sa.
    apply Sb. *)
  ...
  exact Sc.
  (* And the proof is already finished! Do we need the following? *)
  (* apply EqBi.
  apply n4_3.
  apply EqBi.
  apply n4_3. *)
